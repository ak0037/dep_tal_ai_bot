...
namespace PsiBot.Services.Bot
{
    ...
    public class MediaFrameSourceComponent : ISourceComponent
    {
        ...
        public MediaFrameSourceComponent(Pipeline pipeline, CallHandler callHandler, IGraphLogger logger)
        {
            ...
            this.ScreenShare = this.pipeline.CreateEmitter<Dictionary<string, (Shared<Image>, DateTime)>>(this, nameof(this.ScreenShare));
        }
        ...
        public Emitter<Dictionary<string, (Shared<Image>, DateTime)>> ScreenShare { get; }
        ...
        public void Received(AudioMediaBuffer audioFrame)
        {
            ...
            if (audioFrame.UnmixedAudioBuffers != null)
            {
                foreach (var buffer in audioFrame.UnmixedAudioBuffers)
                {
                    ...
                    // Console.Write($"audio:- {BitConverter.ToString(data)}");
        ...
        public static List<AudioMediaBuffer> CreateAudioMediaBuffers(byte[] buffer, long currentTick)
        {
            var audioMediaBuffers = new List<AudioMediaBuffer>();
            var referenceTime = currentTick;
            // packet size of 20 ms
            var numberOfTicksInOneAudioBuffer = 20 * 10000;
            int bytesPerBuffer = 640; // 16000 * 2 * 0.02 = 640 bytes for 20ms of PCM 16K audio

            for (int offset = 0; offset < buffer.Length; offset += bytesPerBuffer)
            {
                int remainingBytes = Math.Min(bytesPerBuffer, buffer.Length - offset);
                if (remainingBytes < bytesPerBuffer)
                {
                    // Skip the last incomplete buffer
                    break;
                }
                IntPtr unmanagedBuffer = Marshal.AllocHGlobal(bytesPerBuffer);
                Marshal.Copy(buffer, offset, unmanagedBuffer, bytesPerBuffer);
                var audioBuffer = new AudioSendBuffer(unmanagedBuffer, bytesPerBuffer, AudioFormat.Pcm16K, referenceTime);
                audioMediaBuffers.Add(audioBuffer);
                referenceTime += numberOfTicksInOneAudioBuffer;
            }
            Console.WriteLine($"Created {audioMediaBuffers.Count} AudioMediaBuffers");
        ...
        public void ReceivedVbss(VideoMediaBuffer videoFrame, uint id)
        {
            if (videoFrame.Timestamp == 0)
            {
                this.logger.Warn($"VBSS buffer timestamp is zero from sender: {id}");
                return;
            }
            ...
            if (length > videoFrame.Length)
            {
                this.logger.Warn($"Length of VBSS frame not as expected: {id}");
                return;
            }
            ...
            {
                this.logger.Warn($"Couldn't find participant for VBSS media source ID: {id}");
            }

            lock (this.ScreenShare)
            {
                this.ScreenShare.Post(streams, this.pipeline.GetCurrentTime());
            }
        }
