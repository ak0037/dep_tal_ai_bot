...
namespace PsiBot.Services.Bot
{
    ...
    public class BotService : IDisposable, IBotService
    {
        ...
        private ILocalMediaSession CreateLocalMediaSession(Guid mediaSessionId = default)
        {
            try
            {
                Console.WriteLine($"\n[MEDIA] Starting media session creation. SessionId: {(mediaSessionId == default ? "New" : mediaSessionId.ToString())}");

                // Add detailed connection testing
                LogMediaConnectionDetails();
        ...
        private void LogMediaConnectionDetails()
        {
            Console.WriteLine("\n[MEDIA DIAG] Detailed Connection Test:");
            try
            {
                // DNS Resolution
                var ips = Dns.GetHostAddresses(botConfiguration.MediaServiceFQDN);
                Console.WriteLine($"[MEDIA DIAG] DNS Resolution for {botConfiguration.MediaServiceFQDN}:");
                foreach (var ip in ips)
                {
                    Console.WriteLine($"[MEDIA DIAG] - Resolved IP: {ip}");
                }

                // TCP Connection Test
                using (var tcpClient = new TcpClient())
                {
                    Console.WriteLine($"[MEDIA DIAG] Attempting TCP connection to {botConfiguration.MediaServiceFQDN}:{botConfiguration.MediaPlatformSettings.MediaPlatformInstanceSettings.InstancePublicPort}");
                    var connectTask = tcpClient.ConnectAsync(
                        botConfiguration.MediaServiceFQDN,
                        botConfiguration.MediaPlatformSettings.MediaPlatformInstanceSettings.InstancePublicPort
                    );

                    if (Task.WhenAny(connectTask, Task.Delay(5000)).Result == connectTask)
                    {
                        Console.WriteLine($"[MEDIA DIAG] TCP Connection successful from {tcpClient.Client.LocalEndPoint} to {tcpClient.Client.RemoteEndPoint}");

                        // Update the InstancePublicIPAddress
                        var ipAddress = ((IPEndPoint)tcpClient.Client.RemoteEndPoint).Address;
                        botConfiguration.MediaPlatformSettings.MediaPlatformInstanceSettings.InstancePublicIPAddress = ipAddress;
                        Console.WriteLine($"[MEDIA DIAG] Updated InstancePublicIPAddress to: {ipAddress}");
                    }
                    else
                    {
                        Console.WriteLine("[MEDIA DIAG] TCP Connection timed out");
                    }
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[MEDIA DIAG] Error during connection test: {ex.GetType().Name} - {ex.Message}");
                if (ex.InnerException != null)
                {
                    Console.WriteLine($"[MEDIA DIAG] Inner error: {ex.InnerException.Message}");
                }
            }
        }
