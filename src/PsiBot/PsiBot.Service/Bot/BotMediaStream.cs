...
namespace PsiBot.Services.Bot
{
    ...
    public class BotMediaStream : ObjectRootDisposable
    {
            ...
            : base(logger)
        {
            ...
            this.logger = logger;
            Console.WriteLine("[MEDIA_STREAM] Initializing BotMediaStream");
            ...
            this.mediaFrameSourceComponent.ScreenShare.PipeTo(teamsBot.ScreenShareIn);
            ...
            teamsBot.AudioOut?.Do(buffer =>
            {
                if (this.audioSendStatus == MediaSendStatus.Active && teamsBot.EnableAudioOutput)
                {
                    if (buffer.Length > 0)
                    {
                        Console.WriteLine($"Incoming audio buffer: Length={buffer.Length}, Format={buffer.Format}");
                        var currentTick = DateTime.Now.Ticks;
                        var tr = MediaFrameSourceComponent.CreateAudioMediaBuffers(buffer.Data, currentTick);
        ...
        public void Subscribe(MediaType mediaType, uint mediaSourceId, VideoResolution videoResolution, uint socketId = 0)
        {
            try
            {
                ...
                this.logger.Info($"Subscribing to the {mediaType} source: {mediaSourceId} on socket: {socketId} with the preferred resolution: {videoResolution}");
            ...
            {
                this.logger.Error(ex, $"{mediaType} Subscription failed for the socket: {socketId} and MediaSourceId: {mediaSourceId} with exception");
            }
        }

        /// <summary>
        /// Unsubscribe from video or vbss.
        /// </summary>
        /// <param name="mediaType">vbss or video.</param>
        ...
        public void Unsubscribe(MediaType mediaType, uint socketId = 0)
        {
            ...
            {
                this.ValidateSubscriptionMediaType(mediaType);
                this.logger.Info($"Unsubscribing from {mediaType} for the socket: {socketId}");
            ...
            {
                this.logger.Error(ex, $"Unsubscribing from {mediaType} failed for the socket: {socketId} with exception");
            }
        }
        ...
        private void OnAudioSendStatusChanged(object sender, AudioSendStatusChangedEventArgs e)
        {
            ...
            Console.WriteLine($"Audio send status changed from {this.audioSendStatus} to {e.MediaSendStatus}");
        ...
        private void SendAudio(AudioMediaBuffer buffer)
        {
            ...
            try
            {
                Console.WriteLine($"Sending audio buffer: Length={buffer.Length}, Timestamp={buffer.Timestamp}");
        ...
        {
            this.logger.Info($"[VbssMediaReceivedEventArgs(Data=<{e.Buffer.Data.ToString()}>, Length={e.Buffer.Length}, Timestamp={e.Buffer.Timestamp}, Width={e.Buffer.VideoFormat.Width}, Height={e.Buffer.VideoFormat.Height}, ColorFormat={e.Buffer.VideoFormat.VideoColorFormat}, FrameRate={e.Buffer.VideoFormat.FrameRate} MediaSourceId={e.Buffer.MediaSourceId})]");
            this.mediaFrameSourceComponent.ReceivedVbss(e.Buffer, e.Buffer.MediaSourceId);
            e.Buffer.Dispose();
        }
